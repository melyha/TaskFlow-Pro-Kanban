...
export const createColumn = (data = {}) => ({
  id: data.id || `column_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
  ...
  const todoColumn = createColumn({
    ...
    tasks: [
      createTask({
        ...
        description: 'Start organizing your tasks in this beautiful Kanban board. Drag and drop tasks between columns.',
        priority: 'medium'
      }),
      createTask({
  ...
  const progressColumn = createColumn({
    ...
    tasks: [
      createTask({
        ...
        description: 'Add more columns, set priorities, and organize your work the way you want.',
        priority: 'high'
      })
    ]
  });
  ...
  const doneColumn = createColumn({
    ...
    tasks: [
      createTask({
        ...
        description: 'You\'ve already seen the beautiful design and smooth interactions!',
        priority: 'none',
        completed: true
      })
    ]
